package aerys.minko.type.parser.collada.resource.effect.technique
{
	...
	public class ConstantTechnique implements ITechnique
	{
		...
		public static function createFromXML(xml : XML, document : ColladaDocument) : ConstantTechnique
		{
			var constant : ConstantTechnique = new ConstantTechnique();
			
			constant._emission			= CommonColorOrTextureOrParam.createFromXML(
                xml.NS::emission[0], document
            );
			constant._reflective		= CommonColorOrTextureOrParam.createFromXML(
                xml.NS::reflective[0], document
            );
			constant._reflectivity		= parseFloat(xml.NS::reflectivity[0].NS::float[0]);
			constant._transparent		= CommonColorOrTextureOrParam.createFromXML(
